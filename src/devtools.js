(function () { 'use strict';
  ...
  const { r, l, condition, component, mount } = radi;
        ...
        l(this.list).loop((component) => {
          return r('div.raddebug-component',
            ...
            condition(l(this.show === component.id),
              r('ul',
                l(component.list).loop((data) => {
                  return r('li',
                    l(data.key),
                    ': ',
                    condition(
                      l(Array.isArray(data.value)),
                      r('strong', 'Array of ', l((data.value).length), ' items')
                    ),
                    condition(
                      l(!Array.isArray(data.value)),
                      r('strong', l(data.value))
                    )
                  );
                })
              )
      ...
      loadList(cp) {
        ...
        for (var ii = 0; ii < cp.length; ii++) {
          ...
          if (!cp[ii]._radi_dvtls) {
            var arr = {id: cp[ii].$id, name: cp[ii].$name, list: []};
            for (var nn in cp[ii].$data) {
              arr.list.push({key: nn, value: cp[ii][nn]});
            }
            comp.push(arr);
          }
        }
