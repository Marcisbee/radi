(function() {
  // eslint-disable-line
  if (typeof $Radi === 'undefined') {
    console.warn('[Radi.js DEV] Err: Radi.js package not found');
    return false;
  }

  const LOGO =
    'data:image/png;base64,';

  const { r, l, list, component, mount, cond } = $Radi;

  mount(
    r(
      'style',
      `.raddebug-wrap {
        position: fixed;
        top: 10px;
        right: 10px;
        bottom: 10px;
        width: 260px;
        max-width: 100%;
        z-index: 10000;
        background-color: rgba(72, 59, 106, 0.97);
        color: #fff;
        border-radius: 6px;
        padding: 14px 16px;
        box-sizing: border-box;
        overflow-x: hidden;
        overflow-y: auto;
        font-family: Tahoma;
        font-size: 16px;
      }
      .raddebug-body {
        width: 100%;
      }
      .raddebug-logo {
        display: block;
        margin: 8px 0 14px 0;
        text-align: center;
      }
      .raddebug-component {
        display: block;
        margin-top: 10px;
        background-color: rgba(44, 37, 64, 0.7);
        border-radius: 6px;
        line-height: 130%;
      }
      .raddebug-component > strong {
        display: block;
        cursor: pointer;
        padding: 5px 10px;
        border-radius: inherit;
        font-size: 14px;
        font-weight: normal;
      }
      .raddebug-component > strong > i {
        opacity: 0.2;
      }
      .raddebug-component > strong:hover {
        background-color: #2d273e;
      }
      .raddebug-component > ul {
        display: block;
        margin: 0;
        padding: 0 10px 10px 10px;
        font-size: 14px;
        list-style: none;
      }
      .raddebug-component ul li {
        white-space: nowrap;
        overflow: hidden;
        text-overflow: ellipsis;
        color: #ae94c5;
      }
      .raddebug-component ul li > strong {
        display: inline;
        padding-left: 6px;
        color: #e2d7ff;
        font-weight: normal;
      }
      .raddebug-state-actor {
        display: block;
        margin-bottom: 10px;
      }
      .raddebug-state-actor button {
        display: block;
        width: 100%;
        margin-top: 10px;
        cursor: pointer;
      }`
    ),
    document.head
  );

  const stateActor = component({
    view() {
      return r(
        'div',
        { class: 'raddebug-state-actor' },
        'State Actor: ',
        cond(
          l(!this.paused),
          r(
            'button',
            {
              onclick() {
                this.paused = true;
                $Radi.freeze();
              },
            },
            '■ Pause state'
          )
        ).else(
          r(
            'button',
            {
              onclick() {
                $Radi.unfreeze();
                this.paused = false;
              },
            },
            '► Resume state'
          )
        )
      );
    },
    state: {
      _radi_no_debug: true,
      paused: false,
    },
    actions: {
      pause() {
        this.paused = true;
        $Radi.freeze();
      },
      resume() {
        $Radi.unfreeze();
        this.paused = false;
      },
    },
  });

  window.treeList = component({
    name: 'tree-list',
    // view: () => ( r('div', '>') )
    view() {
      return r('div', list(l(this.array), () => r('div', 'item')));
    },
    props: {
      array: Array,
    },
    actions: {
      onMount() {
        console.log('me mount', this.array);
      },
    },
  });

  const data = component({
    view() {
      return r(
        'div',
        list(l(this.list), listComponent =>
          r(
            'div',
            { class: 'raddebug-listComponent' },
            r(
              'strong',
              { onclick: () => this.toggle(listComponent.id) },
              l(listComponent.name ? listComponent.name : `unnamed-${listComponent.id}`)
            ),
            cond(
              l(this.show === listComponent.id),
              r(
                'ul',
                cond(l(listComponent.vars.state.length), r('strong', 'State')),
                list(l(listComponent.vars.state), item =>
                  r(
                    'li',
                    l(item.key),
                    ': ',
                    cond(l(Array.isArray(item.value) && item.value !== null)).else(
                      r('strong', l(item.value))
                    )
                  )
                ),
                cond(l(listComponent.vars.props.length), r('strong', 'Props')),
                list(l(listComponent.vars.props), item =>
                  r(
                    'li',
                    l(item.key),
                    ': ',
                    cond(
                      l(Array.isArray(item.value) && item.value !== null),
                      r('strong', 'Array of ', l(item.value.length), ' items')
                    ).else(r('strong', l(item.value)))
                  )
                ),
                cond(l(listComponent.vars.actions.length), r('strong', 'Actions')),
                list(l(listComponent.vars.actions), item =>
                  r(
                    'li',
                    l(item.key),
                    ': ',
                    r(
                      'button',
                      {
                        style: {
                          cursor: 'pointer',
                        },
                        onclick: () => {
                          item.value();
                        },
                      },
                      'trigger'
                    )
                  )
                )
              )
            )
          )
        )
      );
    },
    state: {
      _radi_no_debug: true,
      show: null,
      list: [],
    },
    actions: {
      onMount() {
        setInterval(() => {
          this.list = this.loadList($Radi.activeComponents);
        }, 10);
      },
      loadList(cp) {
        const comp = [];
        for (let ii = 0; ii < cp.length; ii++) {
          // Do not debug the debugger
          if (!cp[ii]._radi_no_debug) {
            const arr = {
              id: cp[ii].$id,
              name: cp[ii].$name,
              vars: {
                state: [],
                props: [],
                actions: [],
              },
            };
            /* eslint-disable */
            for (var nn in cp[ii].$state) {
              arr.vars.state.push({ key: nn, value: cp[ii][nn] });
            }
            for (var nn in cp[ii].$mixins) {
              arr.vars.state.push({ key: nn, value: cp[ii][nn], t: 'mix' });
            }
            for (var nn in cp[ii].$props) {
              arr.vars.props.push({ key: nn, value: cp[ii][nn] });
            }
            for (var nn in cp[ii].$actions) {
              arr.vars.actions.push({ key: nn, value: cp[ii][nn] });
            }
            /*eslint enable*/
            comp.push(arr);
          }
        }

        // setTimeout(() => {
        //   this.loadList(radi.activeComponents)
        // }, 1000)
        return comp;
      },
      toggle(id) {
        console.log(id);
        this.show = this.show === id ? null : id;
      },
    },
  });

  mount(
    r(
      'div',
      { class: 'raddebug-wrap' },
      r(
        'div',
        { class: 'raddebug-body' },
        r(
          'div',
          { class: 'raddebug-logo' },
          r('img', {
            src: LOGO,
            width: 107,
          })
        ),
        r('div', new stateActor()),
        r('div', new data())
      )
    ),
    document.body
  );
})();
